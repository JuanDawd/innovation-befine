...
export default async function AdminCraftablesPage() {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6 lg:p-8">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/craftables/new" className={cn(buttonVariants())}>
          <PlusIcon className="h-4 w-4 mr-2" />
          {t("createCraftable")}
        </Link>
      </div>
      <CraftablesDashboardTable isAdmin={true} />
      <CraftableApprovalBoard isAdmin={true} />
    </div>
  );
}
