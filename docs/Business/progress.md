...
| T09R-R4  | Fix: H-38 — add explicit `method: "POST"` / `"PUT"` / `"DELETE"` matchers with `NetworkOnly` handler in `workboxOptions.runtimeCaching` so mutating requests are never cached. Default-reliance is brittle as Workbox updates change behavior.                                                                                                                                                                                                        | High     | pending | Opus audit               |
