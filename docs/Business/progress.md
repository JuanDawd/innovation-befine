...
| T10R-R2  | Fix: implement `useOptimistic` for ticket status transitions; add revert-on-error test (H-43)         | pending | High     | Opus audit |
| T10R-R3  | Fix: execute Neon PITR restore drill on staging; fill result table in `backup-policy.md` (H-44)       | pending | High     | Opus audit |
